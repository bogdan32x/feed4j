...
/**
 * Base class for building specific feed type parser. It contains also some static utilities.
 * 
...
class TypeAbstract {
     ...
     * Valid local codes.
     */
    private static final String[] VALID_LANGUAGES = new String[] { "af", "sq", "eu", "be", "bg", "ca", "zh-cn", "zh-tw", "hr", "cs", "da", "nl", "nl-be", "nl-nl", "en", "en-au",
            "en-bz", "en-ca", "en-ie", "en-jm", "en-nz", "en-ph", "en-za", "en-tt", "en-gb", "en-us", "en-zw", "et", "fo", "fi", "fr", "fr-be", "fr-ca", "fr-fr", "fr-lu", "fr-mc",
            "fr-ch", "gl", "gd", "de", "de-at", "de-de", "de-li", "de-lu", "de-ch", "el", "haw", "hu", "is", "in", "ga", "it", "it-it", "it-ch", "ja", "ko", "mk", "no", "pl",
            "pt", "pt-br", "pt-pt", "ro", "ro-mo", "ro-ro", "ru", "ru-mo", "ru-ru", "sr", "sk", "sl", "es", "es-ar", "es-bo", "es-cl", "es-co", "es-cr", "es-do", "es-ec", "es-sv",
            "es-gt", "es-hn", "es-mx", "es-ni", "es-pa", "es-py", "es-pe", "es-pr", "es-es", "es-uy", "es-ve", "sv", "sv-fi", "sv-se", "tr", "uk" };

    /**
     * This method fills a RawElement object recursively, extracting informations from a dom4j Element object.
     * 
     * @param re
     *            The RawElement (side-effect on this one).
     * @param e
     *            The dom4j Element object to explore in order to populate the RawElement supplied.
     */
    protected static void populateRawElement(RawElement re, Element e) {
        ...
        // Attributes.
        for (Iterator<Attribute> i = e.attributeIterator(); i.hasNext();) {
    ...
    /**
     * XML nodes copier. This method populates r1 with the attributes and the nodes taken from re2.
     */
     ...
     * @param str
     *            The string.
     * @return true if the supplied string represents a valid language code, false otherwise.
     */
    ...
    /**
     * GUID maker for feed items. This method generates 18 chars GUIDs starting from two integer values. Usually the two values are
     * hashcodes respectively for the whole feed (i.e. the hashcode of the feed URL) and the specific feed item (i.e. the hashcode of the
     * link/permalink in the item, or the hashcode of the item GUID if one is given). It depends on the type of the feed and on the specific
